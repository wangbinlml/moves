...
//动作
var url = "https://www.btmao.cc/list/s-1-wd--letter--year-0-area--order-addtime-p";
(async () => {
    //for (var ab = 1; ab <535; ab++) {
    //for (var ab = 536; ab > 0; ab--) {

        var pageObj = await pageService.findAll(15);
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            try {
                ...
                if (type == "动作片") {
                    ...
                    moveObj.tag_id = 2;
                } else if (type == "剧情片" || type == "爱情片" || type == "战争片") {
                    moveObj.tag_id = 3;
                } else if (type == "喜剧片") {
                    moveObj.tag_id = 4;
                } else if (type == "恐怖片") {
                ...
                if (downloadList.length > 0 || playList.length > 0) {
                    ...
                    if (exits == false) {
                        ...
                        moveObj = await moveService.insert2(conn, [moveObj.category_id, tag_id, moveObj.name, year, area, moveObj.sets, moveObj.cover, moveObj.source, moveObj.description, moveObj.creator_id]);
