...
module.exports.saveLoginLog = async (req) => {
    ...
    log.info("客户端用户IP：", ip);
    await mysql.query("insert into bs_login_log(user_id,user_name,name,ip,locations,login_time) values(?,?,?,?,?,?)", [user.id, user.user_name, user.name, ip, locations, new Date()]);
};
